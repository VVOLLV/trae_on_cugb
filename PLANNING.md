...
| 项目       | 内容                                          |
| -------- | ------------------------------------------- |
| **活动名称** | TRAE on Campus @中国地质大学（北京） AI Coding Meetup |
...
#### Section 7 · About（已迁移至 Footer 揭开层）
- 不在主内容区域独立显示，而是放在 `footer-layer` 中
- 用户向下滚到底时，主内容向上移开，逐层揭开底部信息
- 内容：活动基本信息 + TRAE 品牌信息 + 特别鸣谢 + 版权声明
- **不需要**个人信息/联系方式
...
| 导航栏滚动阴影   | 滚动距离 > 10px | scroll 事件 + class toggle（`passive: true`） | P1  |
| 时间轴节点激活   | 对应章节进入视口    | Intersection Observer                     | P1  |
| 移动端汉堡菜单   | 点击触发        | JS toggle class                           | P1  |
| **Footer 揭开效果** | **滚轮 / 触摸滑动** | **两层叠放 + wheel/touch + RAF lerp 缓动 + translateY** | **P1** |

### 导航设计

- **双层导航结构**：Top Bar（普通文档流，随滚动消失） + Main Navigation（sticky 吸附在顶部）
- **Top Bar**：深色背景，左侧 `TRAE on Campus @中国地质大学（北京）`，右侧辅助链接（概述/复盘/关于）
- **Main Navigation**：`position: sticky; top: 0`，滚动过 Top Bar 后吸附在顶部，滚动时出现 `box-shadow` 阴影
- 导航项：`概述 · 时间线（下拉）· 复盘 · 关于`
- **时间线下拉**：多栏布局（参考 Stanford 风格），左侧「活动阶段」、中间「关键交付物」、右侧「活动流程」
- **hover 动效**：链接下方绿色短线从中心向两侧展开（`::after` 伪元素，`scaleX` 动画），无背景高亮
- **下拉动画**：从 `translateY(8px)` 滑入 + `opacity` 渐变
- 导航链接居中显示，字体 0.95rem / 500 字重
- 移动端：汉堡菜单，点击变形为 X 图标，Escape 键关闭
- 当前章节高亮
...
| --- | --------- | ---------------------------- |
| 背景色 | `#0A0A0A` | `#0A0A0A`                    |
| 阴影  | 无         | `0 2px 16px rgba(0,0,0,0.3)` |
| 边框  | `#1F1F1F` | `#1F1F1F`                    |
...
│   │   ├── base.css            # 重置样式 + 通用选择器
│   │   ├── navigation.css      # 导航栏（双层导航、多栏下拉、底部短线 hover）
│   │   ├── hero.css            # Hero 区域
│   │   ├── timeline.css        # 时间轴
│   │   ├── phase.css           # 阶段样式
...
│   │   ├── about.css           # 关于内容样式
│   │   ├── reveal.css          # Footer 揭开效果（两层叠放滚切）
│   │   ├── icons.css           # 导航栏图标按钮
│   │   └── responsive.css      # 响应式媒体查询
│   └── js/
│       ├── reveal.js            # Footer 揭开：wheel/touch + RAF lerp + 锚点导航
│       ├── main.js             # 导航交互、汉堡菜单
│       ├── counter.js          # 数字 count-up
│       ├── scroll.js           # 章节高亮、懒加载
│       ├── theme.js            # 主题切换（深色/浅色）
│       └── i18n.js             # 中英文国际化
├── reference/                   # 参考网站源文件（Stanford 风格参考站等）
├── uploads/                      # 静态文件上传目录
└── README.md
```
...
页面底部 Footer 采用**两层叠放滚切**的揭开效果：

```
body
├── top-bar / nav (在 page-container 外，保持 sticky 正常)
...
**实现细节**：
- `footer-layer` 绝对定位钉在底部，永远静止
- `main-layer` 绝对定位覆盖在上，设置 `will-change: transform` 开启 GPU 加速
- 监听 `wheel` / `touchstart` / `touchmove` 事件，手动 `translateY` 移动主内容
- 使用 `requestAnimationFrame` + lerp 缓动（easing 0.12）实现丝滑滚动
- `main-layer` 底部 `border-radius: 0 0 20px 20px` 圆角，视觉更柔和
- 检测 `overflow: auto/scroll` 的子元素，自动放行不拦截
- 锚点链接（`a[href^="#"]`）用 `offsetTop` 计算目标位置，`#about` 直达最大偏移量
- resize 时自动修正 offset
...
| About       | **Footer 揭开层** (不在主滚动流中) | **Footer 揭开层** |
...
*规划版本：v1.7 · 更新时间：2026-05-12 · 新增 Footer 揭开效果（两层叠放滚切 + RAF lerp 缓动）*
